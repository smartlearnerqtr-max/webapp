    ...
    <RequireAuth allowedRoles={['admin']}>
      <div className="admin-dashboard-container">
        <section className="admin-hero">
          <div className="admin-hero-decor admin-hero-decor-1">📊</div>
          <div className="admin-hero-decor admin-hero-decor-2">🏫</div>
        ...
        <section className="admin-content-grid">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                ...
                <form className="form-stack" onSubmit={handleSubmit}>
                  ...
                  <label>
                    ...
                    <div style={{display: 'flex', alignItems: 'center', gap: '0.5rem'}}>
                      <input type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Nhập mật khẩu cấp cho giáo viên" />
            ...
            <article className="admin-panel-card">
              ...
              <div className="admin-panel-body no-padding">
                <div className="admin-scrollable-list">
                  {teachersQuery.data?.map((item) => (
                    <div key={item.user.id} className="admin-list-item">
                  ...
                  {!teachersQuery.data?.length && !teachersQuery.isLoading ? <p style={{padding: '1rem'}}>Chưa có giáo viên nào được cấp tài khoản.</p> : null}
                </div>
              </div>
            </article>
          </div>
              ...
              <div className="admin-panel-body no-padding">
                <div className="admin-scrollable-list">
                  {relationshipsQuery.data?.teachers.map((item) => (
                    <div key={item.teacher.user.id} className="admin-list-item">
                      ...
                      <p>Học sinh học cùng GV khác: {item.shared_student_count}</p>
                    </div>
                  ))}
                  {!relationshipsQuery.data?.teachers.length && !relationshipsQuery.isLoading ? <p style={{padding: '1rem'}}>Chưa có dữ liệu liên kết nào.</p> : null}
                </div>
              </div>
            </article>
              ...
              <div className="admin-panel-body no-padding">
                <div className="admin-scrollable-list">
                  {relationshipsQuery.data?.shared_students.map((item) => (
                    <div key={item.student.id} className="admin-list-item">
                  ...
                  {!relationshipsQuery.data?.shared_students.length && !relationshipsQuery.isLoading ? <p style={{padding: '1rem'}}>Chưa có học sinh nào đang học với nhiều giáo viên.</p> : null}
                </div>
              </div>
            </article>
          </div>
